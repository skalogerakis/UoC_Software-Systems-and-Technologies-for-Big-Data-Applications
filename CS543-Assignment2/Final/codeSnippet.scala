...
//    println("Sanity check for crossTrainDataDF: "+crossTrainDataDF.show(10))
    ...
    val model= pipeline.fit(crossTrainDataDF) //need to fit. Use the train Dataframe defined at the beginning of section 4
    val predictionsDF = model.transform(crossTestDataDF) //Produce predictions on the test set. Use method transform.
